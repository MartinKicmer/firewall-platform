...
void PacketBlockerGateway::createL4TCPRule(L4TCPRequest req, bool permit) {
    ...
    try {
        ...
        if (req.save) {
            pr = boost::process::child(
                this->processPath,
                "-rid", std::to_string(req.ID),
                "-l", "L4TCP",
                "-action", permit ? "permit" : "deny",
                "-sport", std::to_string(req.sPort),
                "-dport", std::to_string(req.dPort),
                "-save",
                "-flag", flag,
                "-minWin", std::to_string(req.minWindowSize),
                "-maxWin", std::to_string(req.maxWindowSize)
            );
        } else {
            pr = boost::process::child(
                this->processPath,
...
void PacketBlockerGateway::printRedirectedPackets(std::shared_ptr<WebSocketService> wsService) {
    while (true) {
        ...
        try {
            ...
            std::string line;
            while (!restart.load() && std::getline(outStream, line)) {
...
nlohmann::json PacketBlockerGateway::getLastPDUS(int count, const std::string& layer, bool permit) {
    ...
    try {
        boost::process::child pr(
            ...
            "-l", layer.c_str(),
            "-action", permit ? "permit" : "deny",
            "redirect", count != -1 ? std::to_string(count) : "all",
            boost::process::std_out > outStream
        );
        ...
        std::cerr << "Error while trying to capture last PDUS: " << e.what() << std::endl;
        std::exit(EXIT_FAILURE);
    }
}
    ...
    try {
        ...
        if (req.save) {
            pr = boost::process::child(
                ...
                "-l", "L3",
                "-action", permit ? "permit" : "deny",
                "-sa", source + "/" + std::to_string(sourcePref),
                "-da", dest + "/" + std::to_string(destPref),
                "-ttlMin", std::to_string(req.ttlMin),
                "-ttlMax", std::to_string(req.ttlMax),
                "-proto", req.proto,
                "-allowFrag", std::to_string(req.allowFragments),
                "-tos", std::to_string(req.tos),
                "-save"
            );
        } else {
            pr = boost::process::child(
