...
private:
    ...
    void insertL4TCPRule(std::shared_ptr<FilterRule> l2rule);
    ...
    std::vector<std::shared_ptr<FilterRule>> findL4TCPRuleByAction(bool permit);
    void setupTables();
    ...
    std::vector<std::shared_ptr<FilterRule>> selectL4TCPRules();
    sqlite3* db;
    std::string schema;
};