...
sudo nft add rule inet fw_test input iifname != "lo" counter queue num 0
sudo nft add rule inet fw_test forward counter queue num 0
sudo nft add rule inet fw_test output oifname != "lo" counter queue num 0

echo "[*] Rules applied. Localhost (lo) is EXCLUDED from filtering."
